...
(define (negatives lst)
  (if (null? lst)
   lst
   (if (negative? (car lst))
      (append (list (car lst)) (negatives (cdr lst)))
      (negatives (cdr lst))
    )
  )
)
...
(define (minAndMaxHelper lst min max)
  (cond
    ((null? lst) (list min max))
    ((< (car lst) min) (minAndMaxHelper (cdr lst) (car lst) max))
    ((> (car lst) max) (minAndMaxHelper (cdr lst) min (car lst)))
    (else (minAndMaxHelper (cdr lst) min max))
  )
)
...
(define (getCommonPlaces state1 state2 zips)
  ;(getCommonPlacesHelper state1 state2 zips (list) (list) )
)
